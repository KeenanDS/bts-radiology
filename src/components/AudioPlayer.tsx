...
const AudioPlayer = ({
    ...
    const syncBackgroundMusic = () => {
      ...
      if (isPlaying) {
        ...
        if (bgGainNodeRef.current) {
          ...
          bgGainNodeRef.current.gain.linearRampToValueAtTime(musicVolume / 100, (audioContextRef.current?.currentTime || 0) + 3 // 3 second fade in
          );
        }
        backgroundMusicRef.current.play().catch(err => console.error("Error playing background music:", err));
      } else {
  ...
  return <div className="w-full max-w-[600px] bg-black/80 backdrop-blur-xl rounded-xl p-4 shadow-2xl border border-white/10">
      {audioUrl && <audio ref={audioRef} src={audioUrl} preload="metadata" />}
      
      {backgroundMusicUrl && !isProcessedAudio && <audio ref={backgroundMusicRef} src={backgroundMusicUrl} loop preload="metadata" />}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          {isProcessedAudio && <span className="text-xs text-green-400 mt-1 inline-block">
              Enhanced with intro/outro music
            </span>}
        </div>
        {showDownload && audioUrl && <Button variant="ghost" size="icon" className="text-white/80 hover:text-white hover:bg-white/10 transition-colors" onClick={handleDownload} title="Download audio">
            <Download size={18} />
          </Button>}
      </div>
      
      <div className="mt-4 space-y-2">
        <Slider value={[currentTime]} max={duration || 100} step={1} className="w-full" onValueChange={handleSeek} disabled={!audioUrl} />
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center gap-4">
          <button className="text-white/80 hover:text-white transition-colors" onClick={skipBackward} disabled={!audioUrl}>
            <SkipBack size={20} />
          </button>
          <button className="w-10 h-10 rounded-full bg-white flex items-center justify-center hover:bg-white/90 transition-colors" onClick={togglePlayPause} disabled={!audioUrl}>
            {isPlaying ? <Pause size={20} className="text-black" /> : <Play size={20} className="text-black ml-1" />}
          </button>
          <button className="text-white/80 hover:text-white transition-colors" onClick={skipForward} disabled={!audioUrl}>
            <SkipForward size={20} />
          </button>
        </div>
        <div className="flex items-center gap-2">
          <Volume2 size={18} className="text-white/80" />
          <Slider value={[volume]} max={100} step={1} className="w-20" onValueChange={handleVolumeChange} />
        </div>
      </div>
      
      {backgroundMusicUrl && !isProcessedAudio && <div className="mt-4 pt-3 border-t border-white/10 space-y-2">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Switch checked={backgroundMusicEnabled} onCheckedChange={setBackgroundMusicEnabled} />
                </TooltipTrigger>
                <TooltipContent>
          ...
          {backgroundMusicEnabled && <div className="flex items-center gap-2">
              <span className="text-white/80 text-xs">Volume</span>
              <Slider value={[musicVolume]} max={50} // Limit background music to 50% max
        step={1} className="w-full" onValueChange={handleMusicVolumeChange} disabled={!backgroundMusicEnabled} />
            </div>}
        </div>}
    </div>;
};

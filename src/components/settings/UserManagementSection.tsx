...
const UserManagementSection = () => {
  ...
  return <div>
      <div className="flex items-center justify-between mb-4">
        ...
        {isGlobalAdmin && <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
      ...
      <Table>
        ...
        <TableBody>
          {users.map(user => <TableRow key={user.id}>
              ...
              <TableCell>
                {currentUser?.id !== user.id && isGlobalAdmin ? <Select defaultValue={user.role} onValueChange={(value: UserRoleOption) => updateUserRole(user.id, value)}>
                  ...
                  </Select> : <span className="capitalize">{user.role.replace('_', ' ')}</span>}
              </TableCell>
              <TableCell>{new Date(user.created_at).toLocaleDateString()}</TableCell>
              <TableCell>
                {currentUser?.id !== user.id && <Button variant="outline" size="sm" onClick={() => fetchUsers()}>
                    Refresh
                  </Button>}
              </TableCell>
            </TableRow>)}
